...
import (
	...
	ibcava "github.com/cosmos/ibc-go/v9/modules/light-clients/14-avalanche"
...
func (suite *AvalancheTestSuite) TestExportMetadata() {
	...
	initIteration := ibcava.GetIterationKey(clientStore, height)
	suite.Require().NotEqual(0, len(initIteration))
	initProcessedTime, found := ibcava.GetProcessedTime(clientStore, height)
	suite.Require().True(found)
	initProcessedHeight, found := ibcava.GetProcessedHeight(clientStore, height)
	suite.Require().True(found)
	...
	suite.Require().Equal(ibcava.ProcessedHeightKey(height), gm[0].GetKey(), "metadata has unexpected key")
	...
	suite.Require().Equal(ibcava.ProcessedTimeKey(height), gm[1].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(initProcessedTime, sdk.BigEndianToUint64(gm[1].GetValue()), "metadata has unexpected value")

	suite.Require().Equal(ibcava.IterationKey(height), gm[2].GetKey(), "metadata has unexpected key")
	...
	iteration := ibcava.GetIterationKey(clientStore, updateHeight)
	suite.Require().NotEqual(0, len(initIteration))
	processedTime, found := ibcava.GetProcessedTime(clientStore, updateHeight)
	suite.Require().True(found)
	processedHeight, found := ibcava.GetProcessedHeight(clientStore, updateHeight)
	suite.Require().True(found)
	...
	suite.Require().Equal(ibcava.ProcessedHeightKey(updateHeight), gm[2].GetKey(), "metadata has unexpected key")
	...
	suite.Require().Equal(ibcava.ProcessedTimeKey(updateHeight), gm[3].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(processedTime, sdk.BigEndianToUint64(gm[3].GetValue()), "metadata has unexpected value")

	// check iteration keys
	suite.Require().Equal(ibcava.IterationKey(height), gm[4].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(initIteration, gm[4].GetValue(), "metadata has unexpected value")

	suite.Require().Equal(ibcava.IterationKey(updateHeight), gm[5].GetKey(), "metadata has unexpected key")
