...
func IterateConsensusMetadata(clientStore storetypes.KVStore, cb func(key, val []byte) bool) {
	...
	iter := storetypes.KVStorePrefixIterator(clientStore, []byte(KeyIterateConsensusStatePrefix))
...
// This is useful when validating whether a packet has reached the time specified delay period in the avalanche client's
...
// GetProcessedTime gets the time (in nanoseconds) at which this chain received and processed a avalanche header.
...
// This is useful when validating whether a packet has reached the specified block delay period in the avalanche client's
// verification functions
...
// GetProcessedHeight gets the height at which this chain received and processed a avalanche header.
...
func getAvaConsensusState(clientStore storetypes.KVStore, cdc codec.BinaryCodec, key []byte) (*ConsensusState, bool) {
	bz := clientStore.Get(key)
	if len(bz) == 0 {
		return nil, false
	}
...
// as this is internal avalanche light client logic.
